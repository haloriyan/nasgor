@extends('layouts.user')

@section('title', "Detail")

@php
    use Carbon\Carbon;
    $canEdit = $inventory->status == "DRAFT";
    $supplier = $inventory->supplier;
@endphp
    
@section('content')
<div class="p-8">
    <div class="bg-white rounded-lg flex items-center gap-8 shadow shadow-slate-200 p-8">
        <a href="{{ route('inventory', ['tab' => $inventory->type]) }}" class="flex items-center">
            <ion-icon name="arrow-back-outline" class="text-xl"></ion-icon>
        </a>
        <div class="flex flex-col gap-1 grow">
            <h2 class="text-lg text-slate-700 font-medium">{{ $inventory->label }}</h2>
            {{-- <div class="text-xs text-slate-500">Pembelian oleh {{ $inventory->creator->name }}</div> --}}
        </div>
        @if ($inventory->status == "DRAFT")
            <a href="{{ route('inventory.proceed', $inventory->id) }}" class="bg-green-500 text-white text-sm p-3 px-6 rounded-lg font-medium">
                Terbitkan
            </a>
        @endif
    </div>
    @if ($message != "")
        <div class="bg-green-500 p-4 rounded-lg text-sm text-white mt-8">
            {{ $message }}
        </div>
    @endif

    <div class="grid grid-cols-3 mobile:grid-cols-1 gap-8 mt-8 items-start">

        @if ($inventory->type == "inbound")
            @if ($inventory->movement_id_ref == null)
                <div class="flex flex-col rounded-lg bg-white">
                    <div class="rounded-t-lg bg-primary text-white font-medium p-4 px-8 flex items-center gap-4">
                        <div class="flex grow">Supplier</div>
                        @if ($canEdit)
                            <button class="bg-white text-xs rounded-lg text-primary p-2 px-3 flex items-center gap-2" onclick="toggleHidden('#EditSupplier')">
                                <ion-icon name="create-outline" class="text-lg"></ion-icon>
                                Ubah
                            </button>
                        @endif
                    </div>
                    <div class="p-8">
                        @if ($supplier == null)
                            -
                        @else
                            <div class="flex justify-center">
                                <img 
                                    src="{{ asset('storage/supplier_photos/' . $supplier->photo) }}" 
                                    alt="{{ $supplier->name }}"
                                    class="w-32 aspect-square rounded-xl object-cover"
                                >
                            </div>
                            <div class="text-lg text-slate-700 font-medium mt-6 mb-4">{{ $supplier->name }}</div>
                            <div class="flex items-center gap-4 mt-2">
                                <ion-icon name="person-outline" class="text-slate-500"></ion-icon>
                                <div class="text-xs text-slate-500">PIC</div>
                                <div class="flex grow justify-end text-sm text-slate-700">{{ $supplier->pic_name }}</div>
                            </div>
                            <div class="flex items-center gap-4 mt-2">
                                <ion-icon name="call-outline" class="text-slate-500"></ion-icon>
                                <div class="text-xs text-slate-500">No. Telepon</div>
                                <div class="flex grow justify-end text-sm text-slate-700">{{ $supplier->phone }}</div>
                            </div>
                            <div class="flex items-center gap-4 mt-2">
                                <ion-icon name="mail-outline" class="text-slate-500"></ion-icon>
                                <div class="text-xs text-slate-500">Email</div>
                                <div class="flex grow justify-end text-sm text-slate-700">{{ $supplier->email }}</div>
                            </div>
                            <div class="text-xs text-slate-500 mt-4">Alamat</div>
                            <div class="text-sm text-slate-700 mt-2">{{ $supplier->address }}</div>
                        @endif
                    </div>
                </div>
            @else
                <div class="flex flex-col rounded-lg bg-white">
                    <div class="rounded-t-lg bg-primary text-white font-medium p-4 px-8 flex items-center gap-4">
                        <div class="flex grow">Cabang</div>
                    </div>
                    <div class="p-8">
                        <div class="flex items-center gap-4 mt-2">
                            <ion-icon name="mail-outline" class="text-slate-500"></ion-icon>
                            <div class="text-xs text-slate-500">Diterima dari</div>
                            <div class="flex grow justify-end text-sm text-slate-700">{{ $inventory->origin->branch->name }}</div>
                        </div>
                    </div>
                </div>
            @endif
        @endif

        @if ($inventory->type == "outbound")
            <div class="flex flex-col rounded-lg bg-white">
                <div class="rounded-t-lg bg-primary text-white font-medium p-4 px-8 flex items-center gap-4">
                    <div class="flex grow">Tujuan</div>
                    @if ($canEdit)
                        <button class="bg-white text-xs rounded-lg text-primary p-2 px-3 flex items-center gap-2" onclick="toggleHidden('#EditBranchDestination')">
                            <ion-icon name="create-outline" class="text-lg"></ion-icon>
                            Ubah
                        </button>
                    @endif
                </div>
                <div class="p-8">
                    @if ($inventory->branch_id_destination != null)
                        <div class="flex items-center gap-4 mt-2">
                            <ion-icon name="storefront-outline" class="text-slate-500"></ion-icon>
                            <div class="text-xs text-slate-500">Cabang</div>
                            <div class="flex grow justify-end text-sm text-slate-600">{{ $inventory->branch_destination->name }}</div>
                        </div>
                    @endif
                </div>
            </div>
        @endif

        <div class="flex flex-col rounded-lg bg-white">
            <div class="rounded-t-lg bg-primary text-white font-medium p-4 px-8 flex items-center gap-4">
                <div class="flex grow">Catatan</div>
                @if ($inventory->status == "DRAFT")
                    <button class="bg-white text-xs rounded-lg text-primary p-2 px-3 flex items-center gap-2" onclick="EditNotes({{ $inventory->id }}, '{{ $inventory->notes }}')">
                        <ion-icon name="create-outline" class="text-lg"></ion-icon>
                        Ubah
                    </button>
                @endif
            </div>
            <div class="p-8">
                <div class="text-sm text-slate-600">{{ $inventory->notes == null ? 'Tidak ada catatan' : $inventory->notes }}</div>
            </div>
        </div>

        <div class="flex flex-col rounded-lg bg-white">
            <div class="rounded-t-lg bg-primary text-white font-medium p-4 px-8 flex items-center gap-4">
                <div class="flex grow">Log</div>
            </div>
            <div class="p-8 flex flex-col gap-2">
                <div class="text-xs text-slate-500 mb-1">Dibuat</div>
                <div class="flex items-center gap-2 text-slate-700 text-sm">
                    <ion-icon name="person-outline"></ion-icon>
                    <div class="text-xs">{{ $inventory->user->name }}</div>
                </div>
                <div class="flex items-center gap-2 text-slate-700 text-sm">
                    <ion-icon name="time-outline"></ion-icon>
                    <div class="text-xs">{{ Carbon::parse($inventory->created_at)->isoFormat('DD MMMM YYYY, HH:mm:ss') }}</div>
                </div>
                <div class="text-xs text-slate-500 mt-4">Diperbarui</div>
                <div class="flex items-center gap-2 text-slate-700 text-sm">
                    <ion-icon name="time-outline"></ion-icon>
                    <div class="text-xs">{{ Carbon::parse($inventory->updated_at)->isoFormat('DD MMMM YYYY, HH:mm:ss') }}</div>
                </div>
            </div>
        </div>

    </div>

    <div class="flex flex-col rounded-lg bg-white mt-8">
        <div class="rounded-t-lg bg-primary text-white font-medium p-4 px-8 flex items-center gap-4">
            <div class="flex grow">Produk</div>
            @if ($canEdit)
                <button class="bg-white text-xs rounded-lg text-primary p-2 px-3 flex items-center gap-2" onclick="toggleHidden('#AddProduct')">
                    <ion-icon name="add-outline" class="text-lg"></ion-icon>
                    Tambah
                </button>
            @endif
        </div>
        <div class="min-w-full overflow-hidden overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="text-sm text-slate-700 bg-gray-50">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left"></th>
                        <th scope="col" class="px-6 py-3 text-left">Produk</th>
                        <th scope="col" class="px-6 py-3 text-left">Harga</th>
                        <th scope="col" class="px-6 py-3 text-left">Quantity</th>
                        <th scope="col" class="px-6 py-3 text-left">Total</th>
                        <th scope="col" class="px-6 py-3 text-left"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @foreach ($inventory->items as $i => $item)
                        <tr>
                            <td class="px-6 py-4 text-sm text-slate-700 w-8">
                                {{ $i + 1}}
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-700 flex items-center gap-4">
                                <div>{{ $item->product->name }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-700">
                                {{ currency_encode($item->price) }}
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-700 flex items-center gap-2">
                                <div>{{ $item->quantity }}</div>
                                @if ($canEdit == "DRAFT")
                                    <div class="p-1 px-3 rounded-full text-xs text-green-500 bg-green-100 font-medium cursor-pointer" onclick="EditQuantity('{{ $item }}')">Ubah</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-700">
                                {{ currency_encode($item->total_price) }}
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-700 flex items-center gap-4">
                                @if ($canEdit)
                                    <a href="{{ route('inventory.detail.removeProduct', [$inventory->id, $item->id]) }}" class="bg-red-500 text-white flex items-center p-2 px-3 rounded-lg" onclick="RemoveProduct(event, '{{ $item }}')">
                                        <ion-icon name="trash-outline" class="text-lg"></ion-icon>
                                    </a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
                    <tr>
                        <th class="w-8"></th>
                        <th class="px-6 py-4 text-left text-sm text-slate-700" colspan="2">
                            Subtotal
                        </th>
                        <th class="px-6 py-4 text-left text-sm text-slate-700">{{ $inventory->total_quantity }}</th>
                        <th class="px-6 py-4 text-left text-sm text-slate-700">{{ currency_encode($inventory->total_price) }}</th>
                    </tr>
                </tbody>
            </table>
            {{-- @if ($inventory->type == "inbound")
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="text-sm text-slate-700 bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left"></th>
                            <th scope="col" class="px-6 py-3 text-left">Produk</th>
                            <th scope="col" class="px-6 py-3 text-left">Harga</th>
                            <th scope="col" class="px-6 py-3 text-left">Quantity</th>
                            <th scope="col" class="px-6 py-3 text-left">Total</th>
                            <th scope="col" class="px-6 py-3 text-left"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($inventory->items as $i => $item)
                            <tr>
                                <td class="px-6 py-4 text-sm text-slate-700 w-8">
                                    {{ $i + 1}}
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700 flex items-center gap-4">
                                    <div>{{ $item->product->name }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700">
                                    {{ currency_encode($item->price) }}
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700 flex items-center gap-2">
                                    <div>{{ $item->quantity }}</div>
                                    @if ($canEdit == "DRAFT")
                                        <div class="p-1 px-3 rounded-full text-xs text-green-500 bg-green-100 font-medium cursor-pointer" onclick="EditQuantity('{{ $item }}')">Ubah</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700">
                                    {{ currency_encode($item->total_price) }}
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700 flex items-center gap-4">
                                    @if ($canEdit)
                                        <a href="{{ route('inventory.detail.removeProduct', [$inventory->id, $item->id]) }}" class="bg-red-500 text-white flex items-center p-2 px-3 rounded-lg" onclick="RemoveProduct(event, '{{ $item }}')">
                                            <ion-icon name="trash-outline" class="text-lg"></ion-icon>
                                        </a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
                        <tr>
                            <th class="w-8"></th>
                            <th class="px-6 py-4 text-left text-sm text-slate-700" colspan="2">
                                Subtotal
                            </th>
                            <th class="px-6 py-4 text-left text-sm text-slate-700">{{ $inventory->total_quantity }}</th>
                            <th class="px-6 py-4 text-left text-sm text-slate-700">{{ currency_encode($inventory->total_price) }}</th>
                        </tr>
                    </tbody>
                </table>
            @else
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="text-sm text-slate-700 bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left"></th>
                            <th scope="col" class="px-6 py-3 text-left">Produk</th>
                            <th scope="col" class="px-6 py-3 text-left">Quantity</th>
                            <th scope="col" class="px-6 py-3 text-left"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($inventory->items as $i => $item)
                            <tr>
                                <td class="px-6 py-4 text-sm text-slate-700 w-8">
                                    {{ $i + 1}}
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700 ">
                                    <div>{{ $item->product->name }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700 w-full">
                                    <div class="flex items-center gap-4">
                                        <div>{{ $item->quantity }}</div>
                                        @if ($canEdit == "DRAFT")
                                            <div class="p-1 px-3 rounded-full text-xs text-green-500 bg-green-100 font-medium cursor-pointer" onclick="EditQuantity('{{ $item }}')">Ubah</div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-700 w-full">
                                    <div class="bg-red-200 flex items-center justify-end gap-4">
                                        @if ($canEdit)
                                            <a href="{{ route('inventory.detail.removeProduct', [$inventory->id, $item->id]) }}" class="bg-red-500 text-white flex items-center p-2 px-3 rounded-lg" onclick="RemoveProduct(event, '{{ $item }}')">
                                                <ion-icon name="trash-outline" class="text-lg"></ion-icon>
                                            </a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                        <tr>
                            <th class="w-8"></th>
                            <th class="px-6 py-4 text-left text-sm text-slate-700">
                                Subtotal
                            </th>
                            <th class="px-6 py-4 text-left text-sm text-slate-700">{{ $inventory->total_quantity }}</th>
                        </tr>
                    </tbody>
                </table>
            @endif --}}
        </div>
    </div>
</div>
@endsection

@section('ModalArea')

@include('user.inventory.add_product')
@include('user.inventory.outbound.add_product')
@include('user.inventory.edit_notes')
@include('user.inventory.edit_supplier')
@include('user.inventory.edit_quantity')
@include('user.inventory.remove_product')

@include('user.inventory.outbound.edit_branch_destination')

@endsection

@section('javascript')
<script src="{{ asset('js/MultiSelectorAPI.js') }}"></script>
<script>
    const EditNotes = (id, notes) => {
        select("#EditNotes #id").value = id;
        select("#EditNotes #notes").value = notes;
        toggleHidden('#EditNotes');
    }
    const EditQuantity = data => {
        data = JSON.parse(data);
        select("#EditQuantity #id").value = data.id;
        select("#EditQuantity #quantity").value = data.quantity;
        toggleHidden("#EditQuantity");
    }
    const RemoveProduct = (event, data) => {
        event.preventDefault();

        const link = event.currentTarget;
        data = JSON.parse(data);
        select("#RemoveProduct form").setAttribute('action', link.href);
        select("#RemoveProduct #name").innerHTML = data.product.name;
        toggleHidden("#RemoveProduct");
    }

    new MultiSelectorAPI('#ProductSelector', [], {
        fetchUrl: '/api/product/search?branch_id={{ $inventory->branch_id }}&q=',
        name: "product_ids",
        label: "Produk",
        parseResponse: (data) => data.products // if the response is { categories: [...] }
    });
</script>
@endsection