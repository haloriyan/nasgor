    ...
    <div class="flex items-center gap-8">
        ...
        <div>
            ...
            <div class="flex p-2 bg-slate-200 rounded-lg mt-1">
                <a href="?status=DRAFT" class="p-2 px-6 rounded-lg text-xs {{ $request->status == "DRAFT" ? 'bg-white text-primary' : 'text-slate-500' }}">
                    DRAFT
                </a>
                <a href="?status=" class="p-2 px-6 rounded-lg text-xs {{ $request->status == "" ? 'bg-white text-primary font-medium' : 'text-slate-500' }}">
                    SEMUA
                </a>
                <a href="?status=PUBLISHED" class="p-2 px-6 rounded-lg text-xs {{ $request->status == "PUBLISHED" ? 'bg-white text-primary font-medium' : 'text-slate-500' }}">
                    PUBLISHED
                </a>
            </div>
        </div>
